...
DB_PATH = os.path.join(PROJECT_ROOT, 'database', 'library.db')
PUBLIC_DIR = os.path.join(PROJECT_ROOT, 'frontend')
...
def get_gemini_api_key():
    ...
    for search_dir in [PROJECT_ROOT, os.path.dirname(os.path.abspath(__file__))]:
        key_path = os.path.join(search_dir, 'gemini_key.txt')
...
class LibraryAPIHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def translate_path(self, path):
        ...
        real_public = os.path.abspath(PUBLIC_DIR)
        if not real_target.startswith(real_public):
